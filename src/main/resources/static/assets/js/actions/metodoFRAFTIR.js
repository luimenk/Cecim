function valida() {
    ...
    $.getJSON("/FRAFTIR/idInterno/" + obj["idInternoMuestra"], function (result) {
        swal({
            title: "Error!",
        ...
        if (contador !== 0) {
            ...
            formData.append('fraftir', new Blob([JSON.stringify(obj)], {
...
function validaImprimir(valor){
    window.location = "/FRAFTIR/imprimir/" + valor;
}
...
function cargarTabla() {
    var tbl =
        ...
        '</tfoot>' +
        '<tbody>';
    $.getJSON("/FRAFTIR", function (result) {
        $.each(result, function (i, field) {
            tbl +=
                ...
                '<button type="submit" class="btn btn-link btn-info edit" onclick="validaImprimir(' + field.idFRAFTIR + ')"><i class="fa fa-print"></i></button>' +
                '<button type="submit" class="btn btn-link btn-warning edit" onclick="validaModificar(' + field.idFRAFTIR + ')"><i class="fa fa-edit"></i></button>' +
                '<button type="submit" class="btn btn-link btn-danger remove" onclick="validaEliminar(' + field.idFRAFTIR + ')"><i class="fa fa-times"></i></a>' +
                '</td>' +
                '</tr>';
        });
        tbl += '</tbody>';

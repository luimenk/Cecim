...
function validaImprimir(valor){
    window.location = "/selladoTemperatura/imprimirSelladoTemperatura/" + valor;
}
...
function cargarTabla() {
    var tbl =
        ...
        '</tfoot>' +
        '<tbody>';
    $.getJSON("/selladoTemperatura", function (result) {
        $.each(result, function (i, field) {
            tbl +=
                ...
                '<button type="submit" class="btn btn-link btn-info edit" onclick="validaImprimir(' + field.curvaSelladoId + ')"><i class="fa fa-print"></i></button>' +
                '<button type="submit" class="btn btn-link btn-warning edit" onclick="validaModificar(' + field.curvaSelladoId + ')"><i class="fa fa-edit"></i></button>' +
                '<button type="submit" class="btn btn-link btn-danger remove" onclick="validaEliminar(' + field.curvaSelladoId + ')"><i class="fa fa-times"></i></a>' +
                '</td>' +
                '</tr>';
        });
        tbl += '</tbody>';
        $("#selladoTemperaturaTable").append(tbl);
        $('#selladoTemperaturaTable').DataTable({

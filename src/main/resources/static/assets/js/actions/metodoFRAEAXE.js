...
function validaTerminar() {
    ...
    if (contador !== 0) {
        ...
        var formData = new FormData();
        formData.append("signature", blob1);
        formData.append('fraeaxe', new Blob([JSON.stringify(obj)], {
            type: "application/json"
        }));
...
function validaImprimir(valor) {
    window.location = "/FRAEAXE/imprimir/" + valor;
}
...
function vistaAgregar() {
    const url = document.URL;
    const id = url.substring(url.lastIndexOf('/') + 1);
    $.getJSON("/FRAEAXE/" + id, function (result) {
        if (result.estatus === "terminado") {
            swal("¡Alerta!", "Este ensayo ya fue finalizado", "warning");
        } else {
            window.location = "/agregarFRAEAXE/" + id;
        }
    });
}

function verInfo(valor) {
    window.location = "/verFRAEAXE/" + valor;
}
...
function terminar() {
    ...
    $.getJSON("/FRAEAXE/" + id, function (result) {
        if (result.estatus === "terminado") {
            ...
            window.location = "/terminarFRAEAXE/" + id;
        }
    });
}

function verFirma(valor){
    $.getJSON("/FRAEAXE/individual/" + valor, function (result) {
        swal({
            title: 'Rúbrica',
...
function cargarTabla2() {
    ...
    var tbl =
        ...
        '</thead>' +
        '<tbody>';
    $.getJSON("/FRAEAXE/getAllBy/" + id, function (result) {
        let sumaCiclos = 0;
        let columna = 0;
        let puntero = 0;
        for (let i = 0; i < 56; i++) {
            ...
            if (result[puntero] === undefined) {
                ...
                tbl +=
                    ...
                    '<td class="text-center"><button class="btn btn-info" onclick="verFirma(' + result[puntero].idFRAEAXEDATA + ')"><i class="fa fa-pencil"></i>Ver Firma</button></td>' +
                    '<td class="text-center"><button class="btn btn-danger" onclick="verFoto(' + result[puntero].idFRAEAXEDATA + ')"><i class="fa fa-picture-o"></i>Ver Foto</button></td>';
                if (result[puntero].imagenSeleccionada === "Si"){
                    tbl +=
                        '<td class="text-center"><button class="btn btn-success" onclick="imagenNo(' + result[puntero].idFRAEAXEDATA + ')"><i class="fa fa-check-circle"></i>Si</button></td>';
                } else {
                    tbl +=
                        '<td class="text-center"><button class="btn btn-default" onclick="imagenSi(' + result[puntero].idFRAEAXEDATA + ')"><i class="fa fa-times-circle"></i>No</button></td>';
                }
                tbl +=
                    '<td class="text-center"><button class="btn btn-warning" onclick="modificarData(' + result[puntero].idFRAEAXEDATA + ')"><i class="fa fa-pencil-square-o"></i>Modificar</button></td>' +
                    '</tr>';
        ...
        tbl += '<tfoot>' +
            ...
            '</tr>' +
            '</tfoot>';
        $("#fraeaxeTable2").append(tbl);
        $('#fraeaxeTable2').DataTable({
...
function cargarTabla() {
    var tbl =
        ...
        '</thead>' +
        '<tbody>';
    $.getJSON("/FRAEAXE", function (result) {
        $.each(result, function (i, field) {
            ...
            if (field.estatus === "inicio") {
                tbl +=
                    '<td class="text-center"><button class="btn btn-success" onclick="iniciarProceso(' + field.idFRAEAXE + ')"><i class="fa fa-flag"></i>Iniciar</button></td>' +
                ...
                tbl +=
                    ...
                    '<td class="text-center"><button class="btn btn-danger" onclick="verInfo(' + field.idFRAEAXE + ')"><i class="fa fa-eye"></i>Ver detalles</button></td>' +
                    '<td class="text-center">' +
                    '<button type="submit" class="btn btn-link btn-info edit" onclick="validaImprimir(' + field.idFRAEAXE + ')"><i class="fa fa-print"></i></button>' +
                    '<button type="submit" class="btn btn-link btn-warning edit" onclick="validaModificar(' + field.idFRAEAXE + ')"><i class="fa fa-edit"></i></button>' +
                    '<button type="submit" class="btn btn-link btn-danger remove" onclick="validaEliminar(' + field.idFRAEAXE + ')"><i class="fa fa-times"></i></a>' +
                    '</td>' +
                    '</tr>';
            } else {
                tbl +=
        ...
        tbl += '<tfoot>' +
            ...
            '</tr>' +
            '</tfoot>';
        $("#fraeaxeTable").append(tbl);
        $('#fraeaxeTable').DataTable({
            "pagingType": "full_numbers",
            "lengthMenu": [
                [10, 25, 50, -1],
                [10, 25, 50, "All"]
